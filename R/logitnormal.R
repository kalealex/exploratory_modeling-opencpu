logitnormal_model_check <- function(mu_spec, sigma_spec = "~1", data) {
  ...
  # catch values of inf on logit transform
  logit_trans_vars_mu <- str_match_all(mu_spec, "logit\\(\\s*(.*?)\\s*\\)")[[1]][,2]
  for (var_name in logit_trans_vars_mu) {
    # compute logit transform of variable and add to dataframe
    var <- sym(var_name)
    data <- data %>%
      mutate(
        "{{var}}" := if_else({{var}}==0.0,
                             0.001, # avoid -inf errors by fudging the zeros a bit
                             if_else({{var}}==1.0,
                                     0.999, # avoid inf errors by fudging the ones a bit
                                     {{var}}
                             ),
        ),
        "logit_{{var}}" := qlogis({{var}})
      )
    # replace logit({{var}}) with logit_{{var}} in mu_spec
    mu_spec <- str_replace_all(mu_spec, paste("logit\\(", var_name, "\\)", sep = ""), paste("logit_", var_name, sep = ""))
  }
  logit_trans_vars_sigma <- str_match_all(sigma_spec, "logit\\(\\s*(.*?)\\s*\\)")[[1]][,2]
  for (var_name in logit_trans_vars_sigma) {
    # compute logit transform of variable and add to dataframe
    var <- sym(var_name)
    data <- data %>%
      mutate(
    ...
    # replace logit({{var}}) with logit_{{var}} in sigma_spec
    sigma_spec <- str_replace_all(sigma_spec, paste("logit\\(", var_name, "\\)", sep = ""), paste("logit_", var_name, sep = ""))
  }
